...
namespace triqs_ctint {
  ...
  void solver_core::post_process(params_t const &p) {
    ...
    if (M_tau) {
      M_iw = make_gf_from_fourier_km(make_const_view(block_gf<imtime, matrix_valued>{*M_tau}), G0_iw[0].mesh(), make_zero_tail(G0_iw));
    ...
    if (M3pp_tau) {
      ...
      auto M3_iw_tau     = make_gf_from_fourier_km<0>(make_const_view(*M3pp_tau), iw_mesh, make_zero_tail<0>(*M3pp_tau));
      auto M3pp_ferm_iw  = make_gf_from_fourier_km<1>(make_const_view(M3_iw_tau), iw_mesh_large, make_zero_tail<1>(M3_iw_tau));
    ...
    if (M3ph_tau) {
      auto iw_mesh       = gf_mesh<imfreq>{p.beta, Fermion, p.n_iw_M3};
      auto iw_mesh_large = gf_mesh<imfreq>{p.beta, Fermion, p.n_iw_M3 + p.n_iW_M3};
      auto M3_iw_tau     = make_gf_from_fourier_km<0>(make_const_view(*M3ph_tau), iw_mesh, make_zero_tail<0>(*M3ph_tau));
      auto M3ph_ferm_iw  = make_gf_from_fourier_km<1>(make_const_view(M3_iw_tau), iw_mesh_large, make_zero_tail<1>(M3_iw_tau));
      auto iW_mesh       = gf_mesh<imfreq>{p.beta, Boson, p.n_iW_M3};
