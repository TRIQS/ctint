...
namespace triqs::gfs {
  ...
  /// The maximum's norm of a triqs Green function. Takes element-wise absolute values of the data and returns the maximum of that.
  template <typename Mesh, typename Target> double max_norm(gf_const_view<Mesh, Target> const &G) {
    auto const &dat = G.data();
    auto max_itr    = std::max_element(dat.begin(), dat.end(), [](dcomplex a, dcomplex b) { return std::abs(a) < std::abs(b); });
    return std::abs(*max_itr);
  }
  //FIXME gf_view should be constructable from gf_const_view
  template <typename Mesh, typename Target> double max_norm(gf_view<Mesh, Target> const &G) { return max_norm(make_const_view(G)); }
  ...
  block2_gf<Var_t, Target> make_block2_gf(gf_mesh<Var_t> const &m, block_gf_structure_t const &gf_struct) {
    ...
    for (auto const &bl1 : gf_struct) {
      ...
      for (auto const &bl2 : gf_struct) {
        ...
        if
          constexpr(Target::rank == 4) gf_vec.emplace_back(m, make_shape(bl1_size, bl1_size, bl2_size, bl2_size),
