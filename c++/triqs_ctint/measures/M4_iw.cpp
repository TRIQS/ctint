...
namespace triqs_ctint::measures {
  ...
  // One Monte Carlo measurement of the two-particle Green function, for the block pair (bl1, bl2):
  //
  //   M4[bl1,bl2](iw1,iw2,iw3)(i,j,k,l) += sign * M1[iw2,iw1](j,i) * M2[iw4,iw3](l,k)
  //                                      - sign * M1[iw4,iw1](l,i) * M2[iw2,iw3](j,k) * delta(bl1,bl2)
  //
  // with iw4 = iw1 + iw3 - iw2, M1 = M[bl1], M2 = M[bl2]. The exchange term exists only when both
  // legs sit in the same block, which is the `diagonal` case below. i,j run over bl1, k,l over bl2.
  //
  // Inside the mesh sweep the (i,j) pair fixes both scalars, leaving an outer-product update of the
  // contiguous acc(i,j,:,:) plane -- that plane is what gets vectorized.
         ...
         for (auto const &iw1 : iw_mesh) {
           for (auto const &iw2 : iw_mesh) {
             for (auto const &iw3 : iw_mesh) {
               ...
               // Run geometry. A length of 0 means the block size is only known at run time, which
               // is what fma_run's Len == 0 defers to its runtime argument.
               ...
               // Off the diagonal every (i,j) reads all of M2a, so hold it in vector registers across
               // the loops below: the plane as `tiles` widest vectors plus, when N is odd, one
               // min_simd tail. The tile indices must be compile-time (poet::static_for) or the array
               // spills. At N == 8 this wants 16 of the 32 zmm live and does spill, trading spill
               // stores for not re-loading the operand on every (i,j).
