...
namespace triqs_ctint::measures {
  namespace {
    ...
    void process_inner_loop_fused(const mc_weight_t sign, const auto &M1a, const auto &M2a, const auto &M1b, const auto &M2b, auto &acc,
      ...
      for (const auto i : range(bl1_size))
        for (const auto j : range(bl1_size)) {
          ...
          auto *const RESTRICT acc_ij_ptr = &acc(i, j, 0, 0);

          for (const auto k : range(bl2_size)) {
            auto *const RESTRICT M4_ptr = acc_ij_ptr + k * bl2_size;
    ...
    void process_inner_loop_fallback(const mc_weight_t sign, const auto &M1a, const auto &M2a, const auto &M1b, const auto &M2b, auto &acc,
                                     const auto bl1_size, const auto bl2_size) noexcept {
            ...
            for (const auto k : range(bl2_size)) {
              ...
              if constexpr (bl2_batch >= min_width) {
                ...
                for (; index < truncated_size; index += batch2_t::size) {
                  auto *const RESTRICT m4_ptr = &acc(i, j, k, index);
              ...
              for (; index < bl2_size; ++index) { acc(i, j, k, index) = xsimd::fnma(M2sval, M1b(index, i), acc(i, j, k, index)); }
            }
          }
        }
    }
    ...
    void process_inner_loop(const mc_weight_t sign, const auto &M1a, const auto &M2a, const auto &M1b, const auto &M2b, auto &acc,
                            const auto bl1_size, const auto bl2_size, const auto bl1, const auto bl2) noexcept {
      if (bl1 == bl2) [[unlikely]] {
        if constexpr (bl1_batch == bl2_batch) {
          return process_inner_loop_fused<bl1_batch, true>(sign, M1a, M2a, M1b, M2b, acc, bl1_size, bl2_size);
        } else {
          return process_inner_loop_fallback<bl1_batch, bl2_batch, true>(sign, M1a, M2a, M1b, M2b, acc, bl1_size, bl2_size);
        }
      } else {
        if constexpr (bl1_batch == bl2_batch) {
          return process_inner_loop_fused<bl1_batch, false>(sign, M1a, M2a, M1b, M2b, acc, bl1_size, bl2_size);
        } else {
          return process_inner_loop_fallback<bl1_batch, bl2_batch, false>(sign, M1a, M2a, M1b, M2b, acc, bl1_size, bl2_size);
        }
      }
    }
      ...
      for (const auto &iw1 : iw_mesh) {
        for (const auto &iw2 : iw_mesh) {
          for (const auto &iw3 : iw_mesh) {
            ...
            process_inner_loop<bl1_batch, bl2_batch>(sign, M1a, M2a, M1b, M2b, acc, bl1_size, bl2_size, bl1, bl2);
          }
        }
      }
    };
    ...
    constexpr auto iw3pp_accumulate_kernel = []<int bl1_batch, int bl2_batch>(const mc_weight_t sign, const auto &GM, auto &M3_iw, const auto bl1,
      ...
      for (auto iW : iW_mesh) {
        for (auto iw : iw_mesh) {
          ...
          auto acc       = M3[iW, iw];
          process_inner_loop<bl1_batch, bl2_batch>(sign, M1a, M2a, M1a, M2a, acc, bl1_size, bl2_size, bl1, bl2);
        }
      }
    };
    ...
    constexpr auto dlr2d_iw3ph_accumulate_kernel =
       []<int bl1_batch, int bl2_batch>(const mc_weight_t sign, const auto &M, const auto &GMG, const auto &GM, const auto &MG, auto &M3_iw,
    ...
    constexpr auto dlr2d_iw3pp_accumulate_kernel = []<int bl1_batch, int bl2_batch>(const mc_weight_t sign, const auto &GM, auto &M3_iw,
                                                                                     const auto bl1, const auto bl2) noexcept {
      auto const bl1_size = GM[bl1].target_shape()[0];
      auto const bl2_size = GM[bl2].target_shape()[0];
      auto const GM1      = GM[bl1];
    ...
    constexpr auto full_iw3ph_accumulate_kernel =
       []<int bl1_batch, int bl2_batch>(const mc_weight_t sign, const auto &M, const auto &GMG, const auto &GM, const auto &MG, auto &M3_iw,
         ...
         for (auto mp : M3.mesh()) {
           ...
           auto iw1         = mp1.value();
           auto iw2         = mp2.value();
           const auto M1a   = M1[closest_mesh_pt(iw2, iw1)];
           const auto &M2a  = GMG2;
           const auto M1b   = GM1[iw1];
           const auto M2b   = MG2[iw2];
           auto acc         = M3[mp];
    ...
    constexpr auto full_iw3pp_accumulate_kernel = []<int bl1_batch, int bl2_batch>(const mc_weight_t sign, const auto &GM, auto &M3_iw,
                                                                                    const auto bl1, const auto bl2) noexcept {
      auto const bl1_size = GM[bl1].target_shape()[0];
      auto const bl2_size = GM[bl2].target_shape()[0];
      auto const GM1      = GM[bl1];
  ...
  namespace simd {
    ...
    void iw3ph_accumulate(const mc_weight_t sign, const auto &M, const auto &GMG, const auto &GM, const auto &MG, auto &M3_iw, const auto bl1,
      ...
      kernel_dispatch<iw3ph_accumulate_kernel>(bl2_size, sign, M, GMG, GM, MG, M3_iw, bl1, bl2);
    }
    ...
    void iw3pp_accumulate(const mc_weight_t sign, const auto &GM, auto &M3_iw, const auto bl1, const auto bl2, const auto bl2_size) noexcept {
      kernel_dispatch<iw3pp_accumulate_kernel>(bl2_size, sign, GM, M3_iw, bl1, bl2);
    }
    ...
    void dlr2d_iw3ph_accumulate(const mc_weight_t sign, const auto &M, const auto &GMG, const auto &GM, const auto &MG, auto &M3_iw,
      ...
      kernel_dispatch<dlr2d_iw3ph_accumulate_kernel>(bl2_size, sign, M, GMG, GM, MG, M3_iw, bl1, bl2);
    }
    ...
    void dlr2d_iw3pp_accumulate(const mc_weight_t sign, const auto &GM, auto &M3_iw, const auto bl1, const auto bl2,
      ...
      kernel_dispatch<dlr2d_iw3pp_accumulate_kernel>(bl2_size, sign, GM, M3_iw, bl1, bl2);
    }
    ...
    void full_iw3ph_accumulate(const mc_weight_t sign, const auto &M, const auto &GMG, const auto &GM, const auto &MG, auto &M3_iw,
                               const auto bl1, const auto bl2, const auto bl2_size) noexcept {
      kernel_dispatch<full_iw3ph_accumulate_kernel>(bl2_size, sign, M, GMG, GM, MG, M3_iw, bl1, bl2);
    }

    void full_iw3pp_accumulate(const mc_weight_t sign, const auto &GM, auto &M3_iw, const auto bl1, const auto bl2,
                               const auto bl2_size) noexcept {
      kernel_dispatch<full_iw3pp_accumulate_kernel>(bl2_size, sign, GM, M3_iw, bl1, bl2);
    }
