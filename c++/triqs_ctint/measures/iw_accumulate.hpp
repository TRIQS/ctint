...
//            for (long k : range(bl2_size))
//              for (long l : range(bl2_size)) {
//                M4_view(i, j, k, l) += sign * M1_view_val(j, i) * M2_view(l, k);
        ...
        for (long j : range(bl1_size)) {
          ...
          for (; k + inner_simd_size <= bl2_size; k += inner_simd_size) {
            ...
            for (; l + inner_simd_size <= bl2_size; l += inner_simd_size) {
              ...
              M2_view_block = simd::kernel_transpose(M2_view_block);
              ...
              if constexpr (equal_sizes) {
                ...
                for (int m = 0; m < inner_simd_size; ++m) {
                  M4_block[m] = simd::fma_nadd(sign_simd * tmp, simd_t(M2_view_if(j, k + m)), M4_block[m]);
                }
              }
            ...
            for (; l < bl2_size; ++l) {
              ...
              if constexpr (equal_sizes) {
                ...
                tmp = simd::fma_nadd(sign_simd * tmp3, tmp4, tmp);
              }
