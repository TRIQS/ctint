...
namespace triqs_ctint::measures {
  ...
  void iw4pp_accumulate(const mc_weight_t sign, M4_M_t const &M, chi4_iw_v_t &M4pp, const int bl1, const int bl2, const long bl2_size) noexcept {
    ...
    poet::dispatch(
       [&]<int N, int Diag>() {
         ...
         auto const &[iW_mesh, iw_mesh, _] = M4pp(0, 0).mesh();
         auto const &M1                    = M[bl1];
         auto const &M2                    = M[bl2];
         auto const bl1_size               = M1.target_shape()[0];
         auto &acc_bl                      = M4pp(bl1, bl2);

         for (auto iW : iW_mesh) {
           for (auto iw : iw_mesh) {
             // iW - iw is fixed for the whole iwp sweep.
             const auto iW_iw = iW - iw;
             for (auto iwp : iw_mesh) {
               const auto iW_iwp = iW - iwp;
               const auto M1a    = M1[iW_iwp, iw.value()];
               const auto M2a    = M2[iwp.value(), iW_iw];
               const auto M1b    = M1[iwp.value(), iw.value()];
               const auto M2b    = M2[iW_iwp, iW_iw];
  ...
  void M4pp_iw::accumulate(mc_weight_t sign) {
    ...
    for (int bl1 : range(params.n_blocks()))
      for (int bl2 : range(params.n_blocks())) iw4pp_accumulate(sign, M, M4pp_iw_, bl1, bl2, M[bl2].target_shape()[0]);
  }
