...
namespace triqs_ctint {
  ...
  template <bool dag> struct arg_t {
    ...
    bool operator<(arg_t const &x) const {
      return std::tie(tau, u, vertex_label, pos, s) < std::tie(x.tau, x.u, x.vertex_label, x.pos, x.s);
    }
  };
  ...
  template <bool dag>
  std::ostream &operator<<(std::ostream &os, arg_t<dag> const &c) {
    os << (c.dagger ? "cdag_t{" : "c_t{")
       << "tau=" << c.tau << ", "
       << "u=" << c.u << ", "
