...
namespace triqs_ctint {
  ...
  //template <template <typename, typename> class template_type> struct is_gf_view_or_cview : std::false_type {};
  //template <> struct is_gf_view_or_cview<gf_view> : std::true_type {};
  //template <> struct is_gf_view_or_cview<gf_const_view> : std::true_type {};

  // 2D Fourier
  template <template <typename, typename> class gf_view_or_cview, typename M_out, typename M_in, typename T>
  void _fourier_impl(gf_view<cartesian_product<M_out, M_out>, T> const &G_out, gf_const_view<cartesian_product<M_in, M_in>, T> const &G_in) {
    ...
    for (auto const &i : mesh_in) G_1()[_][i]    = _fourier_impl(G_in[_][i]);
    for (auto const &o : mesh_out) G_out()[o][_] = _fourier_impl(make_const_view(G_1[o][_]));
  }

  // 3D Fourier
  template <typename M_out, typename M_in, typename T>
  void _fourier_impl(gf_view<cartesian_product<M_out, M_out, M_out>, T> G_out, gf_const_view<cartesian_product<M_in, M_in, M_in>, T> const &G_in) {
  ...
  std::enable_if_t<is_gf<Gf<imtime, T>>::value, typename Gf<imfreq, T>::regular_type> make_gf_from_fourier_new(Gf<imtime, T> const &G_tau,
                                                                                                               int n_iw = -1) {
  ...
  template <template <typename, typename> class Gf, typename T>
  std::enable_if_t<is_gf<Gf<imfreq, T>>::value, typename Gf<imtime, T>::regular_type> make_gf_from_inverse_fourier_new(Gf<imfreq, T> const &G_iw,
                                                                                                                       int n_tau = -1) {
  ...
  make_gf_from_fourier_new(Gf<cartesian_product<imtime, imtime>, T> const &G_tau, int n_iw1, int n_iw2) {
  ...
  make_gf_from_inverse_fourier_new(Gf<cartesian_product<imfreq, imfreq>, T> const &G_iw, int n_tau1, int n_tau2) {
    return _make_gf_impl_2d<Gf, imfreq, imtime, T>(G_iw, n_tau1, n_tau2);
  }
  ...
  make_gf_from_fourier_new(Gf<cartesian_product<imtime, imtime, imtime>, T> const &G_tau, int n_iw1, int n_iw2, int n_iw3) {
    return _make_gf_impl_3d<Gf, imtime, imfreq, T>(G_tau, n_iw1, n_iw2, n_iw3);
  }
  ...
  make_gf_from_inverse_fourier_new(Gf<cartesian_product<imfreq, imfreq, imfreq>, T> const &G_iw, int n_tau1, int n_tau2, int n_tau3) {
    return _make_gf_impl_3d<Gf, imfreq, imtime, T>(G_iw, n_tau1, n_tau2, n_tau3);
  }
  ...
  template <template <typename, typename> class Gf, typename M_out, typename M_in, typename T>
  typename Gf<cartesian_product<M_out, M_out>, T>::regular_type
  _make_block_gf_impl_2d(typename Gf<cartesian_product<M_in, M_in>, T>::const_view_type const &G_in, int n1, int n2) {
    ...
    if
      constexpr(Gf<M_prod_in, T>::arity == 1) {
        ...
        for (auto const &bl : G_in) G_out_vec.emplace_back(_make_gf_impl_2d<gf_const_view, M_out, M_in, T>(bl(), n1, n2));
    ...
    // block2_gf_const_view
      ...
      for (int i : range(G_in.size1())) {
        ...
        for (int j : range(G_in.size2())) temp_vec.emplace_back(_make_gf_impl_2d<gf_const_view, M_out, M_in, T>(G_in(i, j), n1, n2));
        G_out_vecvec.emplace_back(std::move(temp_vec));
      }
  ...
  template <template <typename, typename> class Gf, typename M_out, typename M_in, typename T>
  typename Gf<cartesian_product<M_out, M_out, M_out>, T>::regular_type
  _make_block_gf_impl_3d(typename Gf<cartesian_product<M_in, M_in, M_in>, T>::const_view_type const &G_in, int n1, int n2, int n3) {
    ...
    if
      constexpr(Gf<M_prod_in, T>::arity == 1) {
        std::vector<gf<M_prod_out, T>> G_out_vec;
        for (auto const &bl : G_in) G_out_vec.emplace_back(_make_gf_impl_3d<gf_const_view, M_out, M_in, T>(bl(), n1, n2, n3));
        typename Gf<M_prod_out, T>::regular_type G_out{G_in.block_names(), G_out_vec};
        return G_out;
      }
    // block2_gf[_const][_view]
      ...
      for (int i : range(G_in.size1())) {
        std::vector<gf<M_prod_out, T>> temp_vec;
        for (int j : range(G_in.size2())) temp_vec.emplace_back(_make_gf_impl_3d<gf_const_view, M_out, M_in, T>(G_in(i, j), n1, n2, n3));
        G_out_vecvec.emplace_back(std::move(temp_vec));
      }
      typename Gf<M_prod_out, T>::regular_type G_out{G_in.block_names(), G_out_vecvec};
      return G_out;
  ...
  make_gf_from_fourier_new(Gf<imtime, T> const &G_tau, int n_iw = -1) {
  ...
  make_gf_from_inverse_fourier_new(Gf<imfreq, T> const &G_iw, int n_tau = -1) {
  ...
  std::enable_if_t<is_block_gf_or_view<Gf<cartesian_product<imtime, imtime, imtime>, T>>::value,
                   typename Gf<cartesian_product<imfreq, imfreq, imfreq>, T>::regular_type>
  make_gf_from_fourier_new(Gf<cartesian_product<imtime, imtime, imtime>, T> const &G_tau, int n_iw1, int n_iw2, int n_iw3) {
    return _make_block_gf_impl_3d<Gf, imfreq, imtime, T>(G_tau, n_iw1, n_iw2, n_iw3);
  }
  ...
  std::enable_if_t<is_block_gf_or_view<Gf<cartesian_product<imfreq, imfreq>, T>>::value,
                   typename Gf<cartesian_product<imtime, imtime, imtime>, T>::regular_type>
  make_gf_from_inverse_fourier_new(Gf<cartesian_product<imfreq, imfreq, imfreq>, T> const &G_iw, int n_tau1, int n_tau2, int n_tau3) {
    return _make_block_gf_impl_3d<Gf, imtime, imfreq, T>(G_iw, n_tau1, n_tau2, n_tau3);
  }
